...
TEST(Telemetry_Payload_V1, has_version_and_field) {
    ...
    EXPECT_TRUE(payload.contains("client_id"));
    EXPECT_TRUE(payload.contains("name"));
    EXPECT_TRUE(payload.contains("unit"));
    EXPECT_TRUE(payload.contains("value"));
...
TEST(Health_payload_V1, has_version_and_field) {
    ...
    EXPECT_TRUE(payload.contains("seq"));
    EXPECT_TRUE(payload.contains("publish_ok"));
    EXPECT_TRUE(payload.contains("publish_fail"));
    EXPECT_TRUE(payload.contains("reconnects"));
    EXPECT_TRUE(payload.contains("timestamp_s"));
    EXPECT_EQ(payload["schema_version"], 1);
}